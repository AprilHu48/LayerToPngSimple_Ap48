	...
	for (var skinName in skins) {
		...
		for (var i = skinLayersCount - 1; i >= 0; i--) {
			...
			if (writePngs) {
				...
				// activeDocument.exportDocument(new File(absImagesDir+"tmp.png"), ExportType.SAVEFORWEB, opts); 
			}
			...
			if (attachmentName == placeholderName) {
				...
				// 	+ '"x":' + x + ',"y":' + y + ',"width":' + Math.round(width) + ',"height":' + Math.round(height) + '}';// //dont need group
				json += '\t"' + placeholderName + '":{'
